...
export const SideNavigation = () => {
  ...
  return <ul className={style.navigation}>
    {links.map(({ name, url }) => {
      return <li key={url} className={classnames({ [style.night]: iseNightMode })}>
        <NavLink to={url} activeClassName={classnames({ [style.activeNavLink]: true, [style.night]: iseNightMode })} >
          {name}
        </NavLink>
      </li>
    })}
